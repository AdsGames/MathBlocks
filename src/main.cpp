...
void Calculate() {
  if (Calc.Calculate(MyBlocks[block1number].GetType(),
                     MyBlocks[block2number].GetType(), mode,
                     numberToMake) == true) {
...
void game() {
  ...
  while (!key[KEY_ESC]) {
    ...
    if (gameScreen == 4) {
      ...
      // Draws text
      textprintf_ex(buffer, font, 320, 420, makecol(0, 0, 0), -1, "Make %i",
                    numberToMake);
      textprintf_right_ex(buffer, font, 620, 0, makecol(255, 255, 255), -1,
                          "Score: %i", score);
      textout_ex(buffer, font, info.c_str(), 20, 0, makecol(255, 255, 255), -1);

      // Draws level up bar
      rectfill(buffer, 150, 10, (score - levelup + 150), 30,
               makecol(255, 0, 0));
      ...
      if (key[KEY_F]) {
        ...
        do {
          ...
          textprintf_right_ex(buffer, font, 620, 0, makecol(255, 255, 255), -1,
                              "Score: %i", score);
          textout_ex(buffer, font, info.c_str(), 20, 0, makecol(255, 255, 255),
                     -1);

          draw_sprite(buffer, menu, 150, 150);
          textprintf_centre_ex(buffer, font, 320, 180, makecol(0, 0, 0), -1,
                               "Final Score: %i", score);
          draw_sprite(buffer, cursors[0], mouse_x - 10, mouse_y - 10);
          draw_sprite(screen, buffer, 0, 0);
          if (mouse_b & 1) {
            if (mouse_x < 260 && mouse_x > 170 && mouse_y < 290 &&
                mouse_y > 260) {
              newgame = 1;
            } else if (mouse_x < 470 && mouse_x > 380 && mouse_y < 290 &&
                       mouse_y > 260) {
              newgame = 0;
            }
          }
      ...
      if (mouse_z) {
        ...
        // else if(mouse_z==4){info = "Multiply";
        // cursors[0]=cursors[3];mode=MULTIPLY;} else if(mouse_z==5){info =
        // "Divide"; cursors[0]=cursors[4];mode=DIVIDE;} else
        // if(mouse_z==6){position_mouse_z(2);info =
        // "Add";cursors[0]=cursors[1];mode=ADD;} else{position_mouse_z(5);info =
        // "Divide";cursors[0]=cursors[4];mode=DIVIDE;}
      }

      // Checks if mouse is in collision with object
      if (mouse_b) {
        for (int i = 0; i < 126; i++) {
          if ((MyBlocks[i].GetX() / 40) * 40 == (mouse_x / 40) * 40 &&
              (MyBlocks[i].GetY() / 40) * 40 == (mouse_y / 40) * 40) {
            if (mouse_b & 1 && mousedown == false && block1 == false &&
                MyBlocks[i].GetType() != 10) {
              block1 = true;
              block1number = i;
              MyBlocks[i].SetSelected(true);
              mousedown = true;
            } else if (mouse_b & 1 && mousedown == false && block1 == true &&
                       MyBlocks[i].GetType() != 10 &&
                       MyBlocks[i].GetSelected() == true) {
              MyBlocks[block1number].SetSelected(false);
              block1 = false;
              mousedown = true;
            } else if (mouse_b & 1 && mousedown == false && block2 == false &&
                       block1 == true && i != block1number) {
              if (i - 1 == block1number || i - 15 == block1number ||
                  i - 14 == block1number || i - 13 == block1number ||
                  i + 1 == block1number || i + 15 == block1number ||
                  i + 14 == block1number || i + 13 == block1number) {
                block2 = true;
                block2number = i;
                Calculate();
            ...
            if (mouse_b & 2) {
              rotate_scaled_sprite(
                  buffer, MyBlocks[i].GetImage(), MyBlocks[i].GetX() - 20,
                  MyBlocks[i].GetY() - 20, itofix(0), itofix(2));
            }
          }
        }
      }
      ...
      for (int i = 0; i < 126; i++) {
        ...
        if (blocks == 126) {
          textout_centre_ex(buffer, font, "You Win!", 320, 240,
                            makecol(0, 0, 0), -1);
        }
      }
...
int main() {
  ...
  delete[] buffer, background, cursors, bimages, foreground, title, help1,
      help2, breaks, menu, font;
